...
最大池化前向要记录每个窗口的最大值位置，反向只把上游梯度写回该位置。Softmax 先减去每行最大值以抑制指数溢出，交叉熵与 Softmax 合并后，logits 梯度可以直接写成概率与 one-hot 标签之差。
...
成品代码中的每个算子都提供前向和反向入口。封装层负责检查 shape、分配输出与临时显存，底层函数只接收指针和维度；`main.cu` 不共享测试状态，单个模块可以独立注释、编译和运行。
