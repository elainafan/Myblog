...
下面的程序用 GPU 计算一维数组的 ReLU。完整地看一遍内存与执行流程，比先背 CUDA 关键字更容易理解。
...
二维矩阵按行连续存储时，让 `threadIdx.x` 对应列通常能得到连续访问。若让相邻线程沿列移动，地址步长变为一整行，转置 kernel 就会出现读写一侧连续、另一侧跨步的问题。后面的 shared memory 转置会专门处理这一点。
