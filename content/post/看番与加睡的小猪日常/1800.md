...
题目大意：这是一个交互题。我想和你玩一个游戏…… 我们为你隐藏了一个有 $n$ 个顶点的环形图（$3 \le n \le 10^{18}$）。环形图是一个无向图，$n$ 个顶点构成一个环。每个顶点都属于这个环。环中顶点的顺序是任意的。你可以通过以下方式进行询问： - "? a b"，其中 $1 \le a, b \le 10^{18}$ 且 $a \neq b$。

数据范围：$3 \le n \le 10^{18}$，$1 \le a,b \le 10^{18}$，$a \ne b$。
...
题目大意：Neko 喜欢因子。在最近的一节数论课上，他从数学老师那里得到了一个有趣的练习题。Neko 有两个整数 $a$ 和 $b$。他的目标是找到一个非负整数 $k$，使得 $a+k$ 和 $b+k$ 的最小公倍数尽可能小。如果有多个最优的 $k$，他需要选择最小的那个。鉴于他的数学天赋，Neko 在这个问题上毫无压力地得到了 Wrong Answer。
