...
有些定义还要求满二叉树的所有叶结点位于同一层。这里把满足这一附加条件的树称为完美二叉树。
...
若把每个空子树都补成外部结点，原有结点变成扩充二叉树的内部结点。外部结点数总比内部结点数多一，因此一棵含 $n$ 个真实结点的非空二叉树共有 $n+1$ 个空链接。
...
完全二叉树中，下标小于 $\lfloor n/2\rfloor$ 的结点都是分支结点，其余结点都是叶结点。自底向上建堆因此从 `n / 2 - 1` 开始。
...
遍历次序还决定操作发生的时机。前序适合在进入结点时完成复制或序列化，后序适合先释放两棵子树再释放根，中序在二叉搜索树上得到有序序列。
...
层序遍历还可以计算每层宽度。处理一轮前先读取队列长度，这些结点恰好来自同一层；本轮加入的孩子留给下一轮。完全二叉树的最后一层可能包含约一半结点，因此层序队列的最坏空间为 $O(n)$ 。
...
重复键必须在接口中约定，可以拒绝、统一放入某一侧，或在结点中保存出现次数。若插入时把相等键放到右侧，检索、删除和中序输出都要遵循相同规则。
...
两个孩子都存在时，可以复制后继的键，再删除右子树最左结点。实际释放的结点至多只有一个孩子。
...
自底向上建堆只调整分支结点。越靠近根的结点虽然可能移动得更远，但数量呈指数减少。总工作量满足
...
优先队列按优先级取出元素，不受元素到达顺序限制。堆能高效支持读取极值、插入和删除极值，因此是优先队列最常见的实现。
