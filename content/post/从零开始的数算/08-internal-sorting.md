...
当多个记录具有相同键时，稳定排序会保留它们原来的相对次序。多级排序可以先按次要键稳定排序，再按主要键稳定排序，从而得到正确的联合顺序。

比较次数只是排序成本的一部分，记录移动、辅助空间、稳定性、输入初始状态和记录本身的大小也会影响实际表现。
...
循环开始时，区间 $[0,i)$ 已经有序；循环结束后，区间 $[0,i]$ 仍包含相同元素并保持有序，这一性质构成算法的循环不变量。
...
对序列 `[5, 2, 4, 6, 1, 3]`，处理 `4` 时有序前缀是 `[2, 5]`。先把 `5` 右移，再把 `4` 放入空位，前缀变成 `[2, 4, 5]`。先移动再插入可以减少对大记录的写入次数。

每把一个元素向左跨过一个较大元素，就恰好消除一个逆序对，所以插入排序的移动次数为 $\Theta(n+I)$ ，其中 $I$ 是输入逆序对数。近乎有序时 $I$ 很小，移动次数也会相应减少。
...
增量序列必须以一结束，否则不同余数类之间永远不会完成比较。除了简单的不断除二，还可以使用 Hibbard、Knuth 等序列避开某些重复工作。Shell 排序的精确界与这些数列有关，不能只根据代码中的两层循环断定为 $O(n^2)$ 。
...
例如记录序列为 `2a, 2b, 1`。第一轮找到 `1` 并与 `2a` 交换，结果成为 `1, 2b, 2a`，两个键为二的记录次序被颠倒。若改为取出最小记录并整体右移中间区间，选择排序可以保持稳定，但移动次数会上升。
...
大量重复键会让普通二路划分反复处理等值元素。三路划分维护小于、等于和大于枢轴的三个区域，等于枢轴的整段不再递归。
...
额外空间为 $O(n+r)$ 。基数排序达到线性复杂度需要 $d$ 和 $r$ 受限；若把任意长整数的位数也计入输入规模，就不能简单省略 $d$ 。
...
若记录之后只用于按序扫描，也可以一直保留下标数组，完全避免移动原记录。代价是每次访问多一次间接寻址，局部性也不如完成重排后的连续数据。
...
任何比较排序在最坏情况下都至少需要 $\Omega(n\log n)$ 次比较。归并排序、堆排序和平均情况下的快速排序达到了这一数量级。计数排序与基数排序利用键的额外结构，因此不受比较决策树下界约束。
