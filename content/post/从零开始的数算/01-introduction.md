...
程序设计要先把自然语言中的人物、动作和限制变成可以精确表示的对象，再由程序处理这些对象。
...
四位整数代替了四个对象的自然语言描述，图中也只保留局面之间能否一步到达。无论之后使用邻接矩阵还是邻接表，采用深度优先还是广度优先，处理的都是同一个图模型。
...
指针本身也是一个值。链表结点中的 `next` 保存后继结点的地址，结点本身仍单独存放。地址关系使逻辑上相邻的结点可以分散在内存各处，也带来了空指针、悬空指针和内存释放等问题。
...
![线性结构、树结构与图结构的逻辑关系对照](assets/01-logical-organization.png)
...
逻辑结构要经过一次存储映射才能落到机器上。同一张线性关系可以映射到连续数组，也可以存成一组带指针的结点。常见的存储组织包括顺序存储、链式存储、索引存储和散列存储。

存储方式由操作决定。顺序表能在常数时间内按下标访问，却要移动元素才能在中间插入；链表已知前驱后只改指针就能插入，却必须从表头逐结点寻找第 $i$ 个元素。脱离具体操作讨论最佳实现没有意义。
...
![四皇后问题按照放置顺序展开的状态空间树](assets/01-nqueens-state-tree.png)
...
复杂度描述输入规模增长时，算法消耗的时间和空间如何变化。分析时忽略机器、编译器和常数因子，只保留增长速度，因而不能精确预测某台机器上的运行时间。
...
大 $O$ 只给出渐近上界。运行时间为 $3n+5$ 的算法属于 $O(n)$ ，也属于 $O(n^2)$ ，但用 $\Theta(n)$ 能更准确地说明其增长率。
...
嵌套执行时复杂度相乘。外层循环运行 $n$ 次，内层每次运行 $m$ 次，总时间为 $O(nm)$ 。若内层次数随外层下标变化，应直接计算求和式，两个独立上界的乘积可能并不紧确。
...
![常见函数随输入规模增长时的数量级差异](assets/01-growth-rates.png)
