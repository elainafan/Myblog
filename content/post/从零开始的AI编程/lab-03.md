...
全连接层和卷积层的主要计算都能归结为矩阵乘。cuBLAS 使用列主序解释矩阵，而 C++ 数组通常按行主序保存，调用 `cublasSgemm` 时要同时处理操作数顺序、转置标志和 leading dimension。shape 正确并不代表布局正确，测试时必须写出每个矩阵的逻辑维度。

卷积无法直接把 NCHW 输入交给 GEMM。`im2col` 会把每个感受野展开为一列，卷积核展开为行，矩阵乘完成后再把结果恢复成输出布局。反向中的 `col2im` 需要把多个窗口对同一输入位置的贡献累加，重叠窗口会带来写冲突。
