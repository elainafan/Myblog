...
1. **定义偏差函数**：
   ...
   若结合律不成立，也就是只要存在不结合的元素 $i^*, j^*, k^*$，必定有 $f(\{i^*\}, \{j^*\}, \{k^*\}) \neq \emptyset$。
...
3. **构造8元组划分**：
   现在假设原本的集合运算不结合，并且已知元素 $i^*, j^*, k^*$ 不结合。
   我们考虑所有可能的子集三元组 $(R,S,T)$ 的空间。
   任取三个**不包含** $i^*, j^*, k^*$ 的子集 $R_0, S_0, T_0$。
   我们定义 $R_1 = R_0 + \{i^*\}$，$S_1 = S_0 + \{j^*\}$，$T_1 = T_0 + \{k^*\}$。（这里 $+$ 即代表把元素加进集合里）。
...
4. **局部证明至少错1个**：
   ...
   回想我们刚才的定义，异或加法有相消的性质：$R_0+R_1 = R_0 + (R_0 + \{i^*\}) = \{i^*\}$（同理得出 $\{j^*\}$ 和 $\{k^*\}$）。
   所以这就等于：
   $$f(\{i^*\}, \{j^*\}, \{k^*\})$$
   前面已知 $f(\{i^*\}, \{j^*\}, \{k^*\}) \neq \emptyset$，这意味着：
