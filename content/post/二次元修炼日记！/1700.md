...
题目大意：交互题。Alice 每轮禁用一种颜色，Bob 需要在 $n\times n$ 网格中选择一个空格并放上另外一种颜色，使任意相邻格子颜色不同。
...
题目大意：给定数组 $a$ 和主题数 $m$ ，选择一个子集，使 $1\sim m$ 中每个数都能整除子集中的某个元素，并最小化所选元素最大值与最小值之差。

数据范围： $1 \leq t \leq 10^4, 1 \leq n,m \leq 10^5, 1 \leq a_i \leq 10^5, \sum n \leq 10^5, \sum m \leq 10^5$
...
题目大意：给定偶数 $n$ 和排列 $p$ ，构造排列 $q$ ，最大化序列 $a_i=p_i+q_i$ 的局部极大值个数。

数据范围： $1 \leq t \leq 10^4, 4 \leq n \leq 10^5, \sum n \leq 10^5$
