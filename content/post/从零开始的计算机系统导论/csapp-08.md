...
- 上下文：程序运行时所需的各种状态信息，包括代码、数据、栈、通用寄存器、程序计数器、环境变量和打开的文件描述符集合。
- 逻辑控制流：程序运行时一系列的``PC值``。
...
需要保存的上下文包括通用寄存器、浮点寄存器、程序计数器、用户栈、状态寄存器、内核栈和各种内核数据结构。内核数据结构中又包括描述地址空间的页表、记录当前进程信息的进程表，以及进程已打开文件的文件表。

- 调度：内核决定抢占当前进程，并决定哪个进程来重新开始。

DMA传输、阻塞系统调用和周期性定时器中断都可能触发调度。例如，进程等待磁盘DMA传输时，内核可以切换到另一个可运行进程，而不是立即返回用户态。
...
- ``运行``：正在CPU上执行，或者等待被内核调度执行。
- ``停止``：执行被信号挂起且不会被调度。收到``SIGSTOP``、``SIGTSTP``、``SIGTTIN``或``SIGTTOU``后，进程保持停止状态，直到收到``SIGCONT``。
- ``终止``：进程永久停止。默认行为为终止的信号、从``main``返回或调用``exit``都会使进程终止；从``main``返回的整数或传给``exit``的``status``会成为退出状态，非0通常表示异常退出。

挂起与停止并不相同。挂起可以是等待资源或时间而暂时无法继续，停止则特指进程收到相关信号后暂停执行。
...
``fork``调用一次却返回两次：父进程得到子进程的PID，子进程得到0，因此可以据此区分两个进程。
...
``waitpid``的参数决定等待集合、返回条件和状态保存位置。

| 参数 | 含义 |
| --- | --- |
| ``pid > 0`` | 等待PID等于``pid``的子进程 |
| ``pid == -1`` | 等待任意子进程 |
...
| 选项 | 行为 |
| --- | --- |
| ``WNOHANG`` | 暂无子进程满足返回条件时立即返回 |
| ``WUNTRACED`` | 子进程终止或停止时返回 |
| ``WCONTINUED`` | 子进程终止，或停止的子进程因``SIGCONT``继续运行时返回 |
| ``0`` | 挂起调用进程，直到子进程退出 |
...
| 宏 | 含义 |
| --- | --- |
| ``WIFEXITED(status)`` | 子进程通过``exit``或从主函数返回而正常终止 |
| ``WEXITSTATUS(status)`` | 在``WIFEXITED``为真时取得退出状态 |
| ``WIFSIGNALED(status)`` | 子进程因未捕获信号而终止 |
| ``WTERMSIG(status)`` | 在``WIFSIGNALED``为真时取得终止信号编号 |
| ``WIFSTOPPED(status)`` | 子进程当前处于停止状态 |
| ``WSTOPSIG(status)`` | 在``WIFSTOPPED``为真时取得停止信号编号 |
| ``WIFCONTINUED(status)`` | 子进程收到``SIGCONT``后继续运行 |
...
| 按键 | 效果 |
| --- | --- |
| ``Ctrl+C`` | 向前台进程组发送``SIGINT``，默认终止前台进程 |
| ``Ctrl+Z`` | 向前台进程组发送``SIGTSTP``，默认停止前台进程并触发调度 |
| ``Ctrl+D`` | 不发送信号；输入行为空时，终端驱动让``read``返回0，程序据此观察到文件结束 |
...
- 终止进程，如``SIGINT、SIGKILL``。
- 终止进程并转储内存，如``SIGILL、SIGFPE、SIGSEGV``。
- 停止进程，直到被``SIGCONT``重新启动，如``SIGSTOP、SIGTSTP``。
- 忽略信号，如``SIGCHLD``。
...
参数``handler``可以取``SIG_IGN``来忽略信号，取``SIG_DFL``来恢复默认行为，也可以传入用户定义的信号处理函数地址。调用处理程序的过程称为捕获信号。
...
``sigprocmask``根据``how``改变当前阻塞集合。

| ``how`` | 操作 |
| --- | --- |
| ``SIG_BLOCK`` | 把``set``中的信号加入``blocked`` |
| ``SIG_UNBLOCK`` | 从``blocked``中移除``set``中的信号 |
...
常见函数在信号处理环境中的安全性可以这样区分。

| 类型 | 例子与原因 |
| --- | --- |
| 非异步安全 | ``malloc``、``free``可能修改全局状态 |
| 非异步安全 | ``printf``、``scanf``可能使用内部缓冲区和锁 |
| 非异步安全 | ``strtok``等不可重入函数可能依赖静态数据 |
| 非异步安全 | ``pthread_mutex_lock``等线程接口可能引发死锁 |
| 异步安全 | ``_exit``、``abort`` |
| 异步安全 | ``sigaddset``、``sigdelset``、``sigemptyset``、``sigfillset`` |
| 异步安全 | ``kill``、``sigaction``、``sigprocmask``、``sigpending`` |
| 异步安全 | ``write``、``read``、``lseek`` |
| 依实现而定 | ``signal``的具体语义与目标平台有关 |
