...
并发错误通常依赖调度顺序，同一程序多次运行可能得到不同结果。一次测试通过不能排除数据竞争（Data Race）。

`Proxy Lab` 接入并发后，需要同时处理多个请求、慢连接、大对象和共享缓存。实现前要明确共享状态及其同步边界。

并发部分不能只背线程 API。先判断控制流会怎样交错，再找哪些内存位置会被多个执行流同时访问，锁和信号量才有明确的保护对象。
