...
三对角矩阵中 $b=1$ 。可以按三条对角线分别存储，也可以为每行保留三个槽位。首尾行可以保留空槽，也可以在地址公式中单独修正。
...
有环广义表不能用普通递归直接计算深度，因为遍历会反复回到已经访问的结点。处理时需要禁止环或增加访问状态，深度等只对树形结构自然成立的操作也要重新定义。
...
回收时间与存活对象量有关，不必扫描大量垃圾；代价是同一时刻只能使用约一半预留空间。分代垃圾回收利用新对象通常很快死亡这一经验，让新对象在较小区域中频繁复制，长期存活对象再晋升到老年代。
...
IP 路由表使用前缀匹配。沿二进制 Radix Tree 读取地址比特时，保存最近遇到的终止结点；无法继续后，返回已经记录的最长前缀。
...
后缀数组 `sa[i]` 保存字典序第 $i$ 小后缀的起始位置。字符串 `banana$` 的后缀按序排列为
...
AVL 要求每棵子树的左右高度差都不超过一。只检查整棵树的左右高度，仍可能让下方某棵子树退化成长链。
...
RR 型与 LL 型对称。插入 `10, 20, 30` 后，对 10 左旋。
...
`rotate` 要根据孩子方向选择左旋或右旋，并完整维护祖父以上的链接。每次旋转都要同步更新父指针和整棵树的根。
...
合并前要求左树的所有键都小于右树。先把左树最大结点伸展到根，此时根不可能有右孩子，再把右树接到它的右侧。
...
均摊界针对任意操作序列的总成本，与随机输入假设无关。某一次操作仍可能很慢，因此不适合要求单次延迟上界的场景。
