...
---
补一下 Edu182，仍然从 B 开始记。

## B
题目大意：给定一个带 0 的排列残缺数组，填完后求最短需要重排的区间长度。

数据范围：见原题。

思路：先用没出现过的数从大到小填 0，然后找第一个和最后一个不在正确位置的下标，二者之间就是需要处理的最短区间。
...
## C
题目大意：给定两个数组，每一列可以选择是否交换，统计让两行都非降的方案数。

数据范围：见原题。

思路：标准 DP。状态表示当前列是否交换，转移时只检查当前列与上一列在两种交换状态下能否保持非降。
...
## D
题目大意：选择一个整数 $x$ 对数组做上取整变换，并考虑变换收益和罚金，求最大收益。

数据范围：见原题。

思路：枚举 $x$。用值域前缀和快速得到每个区间里有多少数会变成同一个值，再减去需要新建该值的罚金。
...
## E1
题目大意：给定数组，统计同时满足左右记录最大值序列约束的子序列数量。

数据范围：见原题。

思路：分别抽出从左看和从右看的记录最大值序列，用 DP 统计前缀/后缀满足这些记录的方案，再枚举最大值出现区间进行合并。
