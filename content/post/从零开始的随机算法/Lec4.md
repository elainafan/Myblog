...
3. **计算期望**：总和 $Z = \sum_{i=1}^n |R_i|$。因为 $a_{ij}$ 是固定的，而 $y_j$ 是随机的， $R_i$ 实际上是 $n$ 次独立抛硬币结果的和（一维随机游走）。当 $n \to \infty$ 时，根据**中心极限定理**， $R_i$ 的分布趋近于正态分布，有 $\mathbb{E}[|R_i|] \approx \sqrt{\frac{2n}{\pi}}$。

4. **得出结论**：因此， $n$ 行的总期望 $\mathbb{E}[Z] \approx n \cdot \sqrt{\frac{2n}{\pi}} = \sqrt{\frac{2}{\pi}} n^{3/2}$。
