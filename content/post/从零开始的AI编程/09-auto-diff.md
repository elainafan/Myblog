...
f'(x)
\approx
\frac{\operatorname{Im}f(x+i\varepsilon)}{\varepsilon}
$$

这里不再计算 $f(x+\varepsilon)-f(x-\varepsilon)$ 这一差值，因而可以使用很小的 $\varepsilon$ 而不遭遇同样的消减误差。它仍有截断误差，也要求整条计算路径支持复数；`abs`、`max`、分支和许多自定义 CUDA 算子无法直接套用这条公式。高阶可微网络可以把 Complex-step 与反向传播组合，用于物理仿真和神经微分方程中的高阶导数。
