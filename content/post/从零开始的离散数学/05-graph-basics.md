...
$$
\overline{E}=\{(u,v) | u,v \in V,u \not = v,(u,v) \notin E\},\quad \overline{G}=\langle V,\overline{E} \rangle
$$
...
设 $v_i$ 是这些邻点中下标最大的一个，则 $i\geq \delta(G)$。于是
...
练手题：设 $D=\langle V,E \rangle$ 为有向简单图， $\delta(D) \ge 2,\delta^{-}(D)>0,\delta^{+}(D)>0,$ 证明：$D$ 中存在长度大于等于 $\mathrm{max}\{\delta^{+}(D),\delta^{-}(D) \}+1$ 的圈。
