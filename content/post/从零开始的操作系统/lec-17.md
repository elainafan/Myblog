...
传统虚拟内存把磁盘映射到比 DRAM 更大的地址空间。类似的间接映射也可以用于 KV cache、远端内存和 GPU memory：
...
RDMA 提供了低延迟 data plane，FaRM 仍要在上层处理 consistency、transactions、failure 和 locality。远程读写变便宜后，这些系统问题依然存在。

## PagedAttention

![PagedAttention](assets/lecture-17/slide-030-pagedattention.png)

LLM inference 同时受计算和 KV cache 管理限制。自回归生成每产生一个 token，都要保存历史 token 的 key/value states。KV cache 的长度随请求动态增长，batch 中每个 request 长度不同，parallel sampling 和 beam search 又会产生共享 prefix。

旧系统常按请求最大长度预分配连续显存，这方便 kernel 访问，却会造成严重内外碎片。实验中只有约 20%-40% 的 KV cache space 真正存放 token states。
...
Block size 决定碎片与管理开销的平衡。block 越小，碎片越少，但 block table 和 kernel 访问 overhead 越高；block 越大，管理更简单，但最后一个 block 的 internal fragmentation 更高。实验中 block size 16 通常表现较好。PagedAttention 会带来约 10%-15% kernel slowdown，但显存效率提升后，整体 serving throughput 仍显著提高。
...
AIFM 同样利用 far memory，重点处理 OS paging 型远程内存的 `semantic gap` 和 `kernel overheads`。OS 只知道 page，不知道应用对象和数据结构；page fault、kernel I/O、busy polling 等路径本身也有不小的开销。

例如应用只查一个 hash table item，OS paging 可能换入整个 page，造成 read/write amplification；OS 也不知道接下来应该 prefetch 哪些对象。RDMA 无法消除这些由 page 粒度产生的浪费。
...
深度学习负载通常分为 training 和 inference。Training 追求 high throughput，inference 追求 low latency/SLO。传统上把两类任务放在不同 GPU 集群里，整体利用率不高。共享 GPU 又受 DL context switch 限制；NVIDIA T4 上从 Train BERT 切到 Infer ResNet 的示例可达约 6s。

PipeSwitch 的目标是 fine-grained time-sharing 多个 DL apps，并把 context switching latency 压到 millisecond-scale。

DL model 具有 layer structure。朴素切换要先通过 PCIe 传完整模型到 GPU，再执行；PipeSwitch 把 model transmission 和 GPU execution 流水化：
...
PipeSwitch 利用模型层次结构重叠传输与执行，再用统一内存管理和 standby worker 隐藏切换中的其余开销。
...
TGS 把任务分成两种角色：`production job` 不能有性能损失，`opportunistic job` 是使用 spare GPU resources 的低优先级任务。资源分配优先保护 production，再把剩余部分交给 opportunistic。
...
TGS 同时追求 transparency、high GPU utilization、performance isolation 和 fault isolation。它把 GPU sharing 从应用框架层下沉到 OS/容器附近，并借助 CUDA unified memory 完成内存迁移。

## 系统层次对比

这些系统在透明性与应用语义之间选择了不同的实现层次：
