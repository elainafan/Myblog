...
在单处理器上，调度器获得控制权通常来自内部事件和外部中断。内核修改 lock 元数据时暂时关闭中断，可以避免在“检查锁空闲”和“把锁标为忙”之间被切走。中断只应在内核短路径中关闭，不能覆盖用户临界区。
...
关中断保护的是 `value` 与等待队列。多处理器上，关闭当前 CPU 的中断并不能阻止其他 CPU 同时访问同一内存位置，所以还需要硬件原子读改写指令。
...
“加入等待队列、标记 sleeping、释放内部保护、切换到其他线程”必须在 sleep/scheduler 路径中原子衔接。所有阻塞同步都要避免这类 lost wakeup 窗口。
...
Semaphore 很适合资源计数，但大型程序里 `P/V` 顺序容易写散。Monitor 把共享数据、访问函数、锁和 condition variable 放在一起管理。锁保护 monitor 内部状态，condition variable 让条件不满足的线程睡在对应队列上。程序只需检查条件是否成立，并保证状态在锁内更新。
...
Semaphore 的 `V` 会积累许可，未来的 `P` 可以消耗；condition variable 的 signal 只对当前等待者有效。因此不能简单把 `semaP` 当作 `cond_wait`，也不能把 `semaV` 当作 `cond_signal`。
