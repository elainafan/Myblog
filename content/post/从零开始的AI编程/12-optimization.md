...
同一层的神经元若以完全相同的权重开始，它们会接收相同梯度并始终保持一致，因此权重通常需要随机初始化。方差太大会让激活和梯度逐层放大，方差太小又会让信号逐层衰减。

Xavier initialization 让前向与反向的方差在近似线性网络中保持稳定，常取
...
ReLU 会截掉一半输入，He initialization 更常使用
...
初始化还要匹配激活函数。SIREN 使用正弦激活，首层权重范围控制输入频率，后续层则按专门尺度初始化，避免高频表示在前向传播中消失。普通 ReLU 网络的初始化规则直接搬到 SIREN 上，激活和梯度分布都会偏离预期。
...
模型有时同时优化准确率、延迟、能耗或多个任务损失。最直接的方法是 scalarization

$$
L(\theta)=\sum_{i=1}^{k}w_iL_i(\theta)
$$

权重 $w_i$ 决定各目标在总梯度中的比例。不同损失的量纲和梯度范数可能相差很大，固定权重并不一定对应预期优先级。

多个任务的梯度也可能互相冲突。若两个梯度内积为负，一步更新会改善其中一个目标却损害另一个。可以把冲突分量投影掉，或者把共享方向与各任务的正交分量分开处理。此类方法仍要明确最终偏好，无法在没有取舍的情况下同时得到每个目标的独立最优解。
