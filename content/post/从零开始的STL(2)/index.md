...
### 输入
第一行一个数 $n(0 < n \leq 100000)$ ，表示格斗场新来的会员数（不包括facer）。以后n行每一行两个数，按照入会的时间给出会员的id和实力值。一开始，facer就是会员，id为1，实力值1000000000。输入保证两人的实力值不同。
...
### 输入
第一行一个数 $n(0 < n \leq 100000)$，表示格斗场新来的会员数（不包括facer）。以后 $n$ 行每一行两个数，按照入会的时间给出会员的id和实力值。一开始，facer就算是会员，id为1，实力值1000000000。
...
所以用set来完成（便于获取最大最小值），虽然multiset也可以，但是set可以去除不必要的元素，所以使用set。
...
2) 在模板中写迭代器时，最好在前面加上typename关键字，否则可能会编译错。VS可能无此问题，但是Dev C++和服务器上的编译环境会有这个问题。 
