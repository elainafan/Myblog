...
全连接层和卷积层的主要计算都能归结为[矩阵乘]({{< ref "06-matrix-product.md" >}})。cuBLAS 使用列主序解释矩阵，而 C++ 数组通常按行主序保存，调用 `cublasSgemm` 时要同时处理操作数顺序、转置标志和 leading dimension。shape 正确并不代表布局正确，测试时必须写出每个矩阵的逻辑维度。

卷积无法直接把 NCHW 输入交给 GEMM。[`im2col`]({{< ref "07-convolution-pooling.md" >}}) 会把每个感受野展开为一列，卷积核展开为行，矩阵乘完成后再把结果恢复成输出布局。反向中的 `col2im` 需要把多个窗口对同一输入位置的贡献累加，重叠窗口会带来写冲突。

最大池化反向要找回前向选中的位置；这份实现没有保存 argmax，而是重新扫描窗口。Softmax 先减去每行最大值以抑制指数溢出，交叉熵与 Softmax 合并后，logits 梯度可以直接写成概率与 one-hot 标签之差。

本次算子同时包含逐元素操作、归约、矩阵乘和数据重排。逐元素 kernel 使用一线程一元素，归约可以先借助 Thrust，矩阵乘交给 cuBLAS，窗口展开与偏置广播则由独立 kernel 完成。
...
当前适配层会创建临时矩阵并做显式转置。源码中的 `temp` 没有在返回前 `cudaFree()`，每调用一次 GEMM 就会泄漏一块结果大小的显存。行主序与列主序之间还满足

$$
(AB)^\mathsf{T}=B^\mathsf{T}A^\mathsf{T}
$$

利用这个关系可以交换操作数和维度，省掉显式转置。

当前每次全连接、卷积都会 `cublasCreate()`，结束后再销毁。这样写起来直接，但初始化开销也进入了每次调用。
...
窗口寻址都放在 `im2col` 中，后面的 GEMM 不再处理 stride、padding 和越界。重叠窗口会重复保存像素，因此临时矩阵可能远大于原输入。
...
当前实现没有保存 argmax，而是在反向时再次比较 `value == max_value`。窗口中出现多个相同最大值时，每个相等位置都会收到完整梯度；这一点与只记录一个最大值下标的实现不同。
...
Softmax 没有手写归约 kernel，而是对 batch 中每一行依次调用 Thrust：先 `reduce` 求最大值，再 `transform` 求指数，第二次 `reduce` 求和，最后归一化。实现很短，不过每一行都会产生多次独立调度。

交叉熵前向让每个样本线程计算自身损失，再用 `atomicAdd` 加到一个标量中。因此调用前必须把 `loss` 清零。若上一轮的 loss 留在显存里，本轮输出会在旧值上继续累加，单看曲线很像训练突然发散。

### 成品代码

最终版本保留 Lab 2 的 `tensor.h` 与 `tensor.cu`，新增算子集中在 `tensor_kernel.h`，五组测试写在 `main.cu`。完整代码见 [Lab 3 源码](https://github.com/elainafan/Programming-in-AI-2025Fall-PKU/tree/main/Lab3)。

### 算子验证

`main.cu` 为五类计算准备固定随机种子和小尺寸输入，分别检查前向、输入梯度、参数梯度与输出形状。卷积、全连接和池化的上游梯度直接填 1，归约结果可以人工核对；Softmax 每行概率和应接近 1，合并后的 logits 梯度每行之和应接近 0。五组测试彼此独立，算子末尾的 `cudaDeviceSynchronize()` 会及时暴露越界，也会让测试中的执行完全串行。

- 全连接层两组输出分别为 $(1.1532,0.9037)$ 和 $(1.3388,1.1262)$ ，偏置梯度为 $(2,2)$ 。
- 卷积输出保持 $3\times4$ ，首行为 $(1.4922,1.7930,1.8034,0.8262)$ ，偏置梯度为 12。
- 最大池化输出为 $(0.9748,0.8535;0.6530,0.6245)$ ，每个 $2\times2$ 窗口只有最大值位置接收单位梯度。
- Softmax 每行概率和为 1，交叉熵对 logits 的梯度每行和为 0。
