...
回归分析用带参数的函数描述变量之间的关系。一元线性回归假设响应变量由线性项和随机误差共同组成。

## 问题设定

在一元模型中， $x_i$ 视为给定的设计点，随机性来自误差项。
...
把模型代回估计量后，估计误差可以写成噪声 $\epsilon_i$ 的线性组合，由此计算无偏性、方差和协方差。
...
拟合值属于模型解释的部分，残差记录观测值与拟合值之间的差。残差平方和还可以估计噪声方差。
...
当误差服从正态分布时，最大化似然等价于最小化残差平方和，因此最小二乘估计也是 $\alpha,\beta$ 的最大似然估计。
...
估计平均响应只包含参数估计的不确定性，预测新的观测值还包含新误差项，因此预测区间更宽。
...
多元线性回归将多个自变量统一写入设计矩阵，估计公式也相应写成矩阵形式。
...
Hat 矩阵 $H$ 把观测向量 $y$ 正交投影到 $X$ 的列空间，投影结果就是拟合值 $\hat{y}$ 。
...
正态误差下，多元回归的分布结论与一元回归一致，标量方差由协方差矩阵取代。
...
当 $X$ 列不满秩或近似不满秩时， $X^\top X$ 可能不可逆或病态。岭回归在最小二乘目标上加入 $\ell_2$ 正则项以稳定估计
...
多元回归中某些列可能是多余特征，直接枚举所有特征子集通常代价很高。LASSO 在残差平方和上加入 $\ell_1$ 正则项

$$
Q_\lambda(\beta)=\|y-X\beta\|_2^2+\lambda\sum_i|\beta_i|.
$$

其中 $\lambda$ 控制正则化强度。 $\ell_1$ 正则项可能使部分系数恰好变为 $0$ ，因此 LASSO 也可用于变量选择。
