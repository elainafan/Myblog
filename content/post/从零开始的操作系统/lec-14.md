...
Lecture 13 的 simple paging 很直观，但线性页表会随 virtual address space 膨胀，本讲先解决这个空间问题。multi-level page table 用树状结构只为实际使用的地址范围分配页表页，不过它也带来新的代价：TLB miss 时 page table walk 会更贵。

页表项本身也不只是 `PPN`。`PTE` 里的 flags 支撑 valid/protection、dirty/reference，以及 demand paging、copy-on-write 和 zero-fill-on-demand。讲完页表后，本讲转向 TLB 和 cache：每次访存都要翻译地址，系统如何避免地址翻译成为主路径瓶颈？

## 本讲地图

| 主题 | 解决的问题 |
| --- | --- |
| Simple page table size | 解释为什么一维线性页表在大地址空间下不可接受 |
| Multi-level page table | 用 sparse tree 压缩大部分无映射地址 |
| PTE flags | 表达 valid/protection/dirty/reference 等状态 |
| x86-64 page table | 展示真实机器中多级索引如何拆分 |
| TLB | 缓存 `VPN -> PPN + protection`，避免每次访问都 walk |
| Cache organization | 用 `tag/index/offset` 比较 direct mapped、set associative 和 fully associative |

## 正文

页表能提供灵活的地址翻译，但它也会变大、变慢。多级页表、PTE、TLB 和 cache 都是在缓解这两个问题。
