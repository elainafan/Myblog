...
性能优化的目标是编写高效的代码，最大限度地利用硬件资源，通常需要考虑三个方面。
...
编译器在代码优化中扮演重要角色，常见操作包括以下几项。

- 执行寄存器的分配和调度
- 代码的重新排序和规划
- 赘余代码的消除

编译器不会引入可能改变程序行为的优化，大多数优化也在函数内部完成。最直接的控制方式是指定优化级别。

- ``-Og``让``GCC``进行基本优化。
- ``-O1,-O2,-O3``让``GCC``进行更大量的优化。

提高优化等级可以改善程序性能，也可能增加程序规模，使标准调试工具更难对程序进行调试，正如在Attack Lab中看到的一样。
...
本节以``Intel Core i7 Haswell``作为参考机。它有八个功能单元。
