...
题目大意：给定数组 $a$ ，多次询问 $(x,y)$ ，求 $a_x+a_{x+y}+a_{x+2y}+\cdots$ 的和。

数据范围： $1 \leq n \leq 3 \cdot 10^5, 1 \leq a_i \leq 10^9, 1 \leq q \leq 3 \cdot 10^5, 1 \leq x,y \leq n$
...
题目大意：给定序列 $a$ ，可以删除任意元素并保持相对顺序，最大化最终序列中满足 $a_i=i$ 的位置数量。
...
题目大意：给定无向图和两对端点及路径长度上限，要求保留两条满足限制的路径并最大化可删除边数。

数据范围： $1 \leq n \leq 3000, 1 \leq m \leq 3000, 1 \leq s_i,t_i \leq n, 0 \leq l_i \leq n$
...
题目大意：给定带障碍网格，每步可以沿一个方向移动 $1$ 到 $k$ 格，求从起点到终点的最少步数。

数据范围： $1 \leq n,m,k \leq 1000, 1 \leq x_1,x_2 \leq n, 1 \leq y_1,y_2 \leq m$
...
题目大意：给定完全图中缺失的边，求补图的连通块个数和每个连通块大小。

数据范围： $1 \leq n \leq 2 \cdot 10^5, 0 \leq m \leq 5 \cdot 10^5$
...
题目大意：动态加入闭区间，每次加入后输出所有区间并集的连通块数量。

数据范围： $1 \leq n \leq 2 \cdot 10^5, 1 \leq l_i < r_i \leq 10^9$
...
题目大意：给定若干回合的卡牌，每回合最多打出总费用不超过 $3$ 的牌，每打出第 $10$ 张牌时伤害翻倍，求最大总伤害。

数据范围： $1 \leq n \leq 2 \cdot 10^5, 1 \leq k_i \leq 2 \cdot 10^5, 1 \leq c_j \leq 3, 1 \leq d_j \leq 10^9, \sum k_i \leq 2 \cdot 10^5$
