...
题目大意：给定混合图，其中部分边已有方向、部分边未定向，判断能否给所有未定向边定向，使最终有向图无环。

数据范围： $1 \leq t \leq 2 \cdot 10^4, 1 \leq n,m \leq 2 \cdot 10^5, \sum n \leq 2 \cdot 10^5, \sum m \leq 2 \cdot 10^5$
...
题目大意：交互题。隐藏整数 $x$ ，每次询问后返回当前 $x$ 除以给定数的向下取整结果，需要确定 $x$ 。

数据范围： $1 \leq t \leq 100, 2 \leq n \leq 10^9$
...
题目大意：交互题。隐藏数组 $a$ ，每次询问长度为 $k$ 的连续段异或和，要求求出整个数组的异或和。

数据范围： $1 \leq t \leq 100, 1 \leq k \leq n \leq 10^5$
...
题目大意：交互博弈题。给定起点和若干点，两人轮流选择未选点移动，根据曼哈顿距离奇偶决定胜负，需要选择先后手并按策略出点。

数据范围： $1 \leq t \leq 10^3, 1 \leq n \leq 2 \cdot 10^5, |x_i|,|y_i| \leq 10^9, \sum n \leq 2 \cdot 10^5$
...
题目大意：给定一个会在平面上无限平铺的网格迷宫，判断能否从起点出发到达同一个模位置的不同平铺副本。

数据范围： $1 \leq n,m \leq 1500, n \cdot m \leq 10^6$
...
题目大意：给定一棵有根树和每个点的颜色，统计合法染色方案数，使每个黑点条件按题面被满足，答案对 $10^9+7$ 取模。

数据范围： $1 \leq n \leq 10^5, c_i \in \{0,1\}$
...
题目大意：给定数组 $a$ 和多组询问 $(p,k)$ ，从位置 $p$ 开始反复令 $p \leftarrow p+a_p+k$ ，求离开数组前的步数。

数据范围： $1 \leq n \leq 10^5, 1 \leq a_i \leq n, 1 \leq q \leq 10^5, 1 \leq p,k \leq n$
...
题目大意：交互题。隐藏一个含 0 和 1 的二进制串，需要在最多 $15$ 次询问内找到一个 0 的位置和一个 1 的位置。
...
题目大意：给定数组 $a$ ，在所有子段中最大化子段和减去该子段最大值。

数据范围： $1 \leq n \leq 10^5, -30 \leq a_i \leq 30$
...
题目大意：给定数组 $a$ 和整数 $k$ ，选择长度为 $k$ 的子序列，最小化奇数位最大值和偶数位最大值中的较小者。

数据范围： $2 \leq k \leq n \leq 2 \cdot 10^5, 1 \leq a_i \leq 10^9$
...
题目大意：给定一棵带点颜色的树，统计端点颜色相同且路径内部不含该颜色的简单路径数量。

数据范围： $1 \leq t \leq 10^4, 2 \leq n \leq 2 \cdot 10^5, 1 \leq c_i \leq n, \sum n \leq 2 \cdot 10^5$
