...
叶结点分裂时，右侧新叶的最小键会复制到父结点作为分隔键，记录仍留在叶子中。内部结点分裂时，分隔键只负责导航，可以提升而不在原结点保留。叶层复制分隔键，内部层提升分隔键。
...
混合型 B+ 树分别规定叶结点和内部结点的阶，两层的容量下限要各自判断。下面的例子中，叶结点阶为 5，至少保存 3 个关键码；内部结点阶为 4，至少拥有 2 个孩子。

删除记录 `33` 后，叶结点 `[33, 45, 47]` 只剩 `[45, 47]`，低于叶层下限。它与右兄弟 `[48, 50, 52]` 合并为 `[45, 47, 48, 50, 52]`，叶链也随之缩短一项。

![混合型 B+ 树删除 33 后，叶结点和父结点出现下溢](assets/11-bplus-deletion-underflow.png)

叶结点合并后，右侧内部结点只剩一个孩子，又低于内部层下限。左侧内部结点仍有三个孩子，可以把最右侧的 `[23, 30, 31]` 移给右侧。重新确定各子树的范围后，根分隔键由 `33` 调整为 `23`，右侧内部结点以 `33` 分隔两个叶结点，整棵树重新满足容量约束。
