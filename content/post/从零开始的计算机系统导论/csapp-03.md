...
大多数指令有一个或多个操作数，用来指出执行操作时使用的源数据值，以及放置结果的目的位置。操作数分为三种类型。
...
除``leaq``外，大部分指令都有对应不同操作数大小的变种。

操作数的顺序与一般直觉相反，通常右边的数写在左边，而左边的数写在右边。

编译器产生的代码通常会用一个寄存器存放多个程序值，也会在寄存器之间传送程序值。
...
CPU除了维护整数寄存器，还维护着一组单个位的条件码寄存器。它们描述了最近的算术或逻辑操作的属性，可以用于执行条件分支指令。常用的条件码有四种。
...
``leaq``不改变任何条件码，其他上述指令都会设置条件码。条件码为1即表示真，反之表示假。
...
它们的操作数顺序是相反的。``cmp``可以测试两个数的大小关系，``test``可以测试某个数是否为负数、零或者正数。
...
假设过程P调用过程Q，Q执行后返回P。过程调用包含下列一种或多种机制。
...
过程通过减小栈指针在栈上分配空间，分配的结果作为栈帧的一部分，标号为局部变量。对象相对于栈指针的偏移量决定了它在栈帧中的位置。
