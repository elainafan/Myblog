...
---
这里放的是 Div.2B 和 Div.1 的 A-D，目录里顺带存着的 Div.2A 不放进来。

## Div.2 B
题目大意：给定数组，判断是否能通过改变符号使第一个数成为中位数。

数据范围：见原题。

思路：只看绝对值。若绝对值小于 $|a_1|$ 的元素个数不超过一半，就可以把 $a_1$ 放到中位数位置。
...
## Div.1 A
题目大意：构造一个 $n\times n$ 的矩阵，把 $0$ 到 $n^2-1$ 按要求填进去。

数据范围：见原题。

思路：从中心开始按右、下、左、上的螺旋顺序填数。这样每一圈都围绕中心展开，直接模拟即可。
...
## Div.1 B
题目大意：给定排列，需要在奇偶位置分别排序，同时保持整体排列可达。

数据范围：见原题。

思路：奇数位和偶数位内部可以各自排序，但两边的逆序奇偶性需要一致。若不一致，排好后交换最后两个同 parity 的位置做一次校正。
...
## Div.1 C
题目大意：给定数组，统计某种前后缀贪心匹配贡献。

数据范围：见原题。

思路：从左到右和从右到左各做一次贪心，用集合维护还没被匹配的值。每个位置的贡献由左侧可匹配数量和右侧可匹配数量的较小值决定。
...
## Div.1 D
题目大意：给定排列，统计满足某种局部区间条件的子区间数量。

数据范围：见原题。

思路：先用线段树求出每个位置能扩展到的左右边界，再把每个位置对应成平面上的矩形事件，用扫描线加离散化线段树统计被覆盖区域。
