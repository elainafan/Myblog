...
定长顺序栈装满后继续 `push` 会发生上溢，空栈上执行 `pop` 或 `top` 会发生下溢。示例用返回值报告失败，也可以抛出异常或在调用前检查，接口约定应保持一致。
...
只保存 `front` 和 `rear` 时，两个下标相等无法区分空队列与满队列。可以额外保存元素数量、记录上一次操作类型，或者始终空出一个槽位。
...
更复杂的递归函数可能在一次调用中包含多个递归点，此时栈帧还要保存阶段编号。改用显式栈不会改变原算法的复杂度，只是把调用状态放进程序能够控制的容器。递归深度可能超过系统栈上限时，显式栈可以避免栈溢出，也便于暂停和恢复搜索。
