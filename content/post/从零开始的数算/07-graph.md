...
深度优先搜索（Depth First Search，DFS）沿未访问的边尽量深入，无法继续时回溯。访问标记可以避免环导致的死循环，并保证每个顶点只被处理一次。
...
DFS 也能生成拓扑序。一个顶点的全部后继处理完毕后再把它加入序列，最后将序列反转。此时需要三色标记区分未访问、正在递归栈中和已经完成的顶点；遇到指向递归栈中顶点的边，说明图中存在环。
...
外层循环必须枚举中间点 $k$ 。若把 $i$ 或 $j$ 放到最外层，原地数组中已经更新与尚未更新的状态会混在一起，无法继续表示仅允许前 $k$ 个中间点的动态规划阶段。
...
把正在构造的生成树记为 $T=(V^\ast,E^\ast)$ 。从任意起点 `s` 开始时，`V*` 只含 `s`，`E*` 为空。随后反复寻找连接树内外顶点的最轻边。加入这条边及其树外端点后，继续下一轮，直到全部顶点都进入 `V*`。
...
`e` 是所有跨界边中最轻的，因此 `w(e) <= w(e')`，`T''` 的总代价不会大于 `T'`。于是 `T''` 仍是 MST，并同时包含 `T` 与 `e`，与假设矛盾。Prim 从一个顶点和零条边开始，每一步都加入一条可以属于某棵 MST 的边，最终得到最小生成树。
...
Kruskal 从没有边的森林开始，把每个顶点看作一个独立的连通分量。所有边按权值从小到大取出。连接不同分量的边加入森林，并合并两个分量；端点已经连通的边会形成环，直接舍去。所有顶点合并为一个分量后，森林便成为 MST。
