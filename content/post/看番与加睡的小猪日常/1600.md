...
题目大意：交互题。隐藏一个长度为 $n$ 的排列 $p$ ，每次询问两个不同位置 $i,j$ 会返回 $p_i \bmod p_j$ ，需要在 $2n$ 次询问内还原整个排列。
...
题目大意：交互题。给定 $n\times m$ 棋盘，隐藏一枚国王；每次询问一个格子会返回到国王的曼哈顿距离，需要在最多 $3$ 次询问内确定国王位置。

数据范围： $1 \leq t \leq 10^3, 1 \leq n,m \leq 10^9$
...
题目大意：给定非负整数 $n$ ，统计有序三元组 $(a,b,c)$ 的数量，使得 $a+b+c=n$ 且十进制加法不产生进位。

数据范围： $1 \leq t \leq 10^4, 0 \leq n \leq 10^7$
...
题目大意：给定一棵 $n$ 点树和整数 $k$ ，删除恰好 $k$ 条边，最大化剩余所有连通块大小的最小值。

数据范围： $1 \leq t \leq 10^4, 1 \leq k < n \leq 10^5, \sum n \leq 10^5$
