...
这一讲用 `FAT`、UNIX/Berkeley `FFS` 和 Windows `NTFS` 三个 case study 串起文件系统布局设计。核心问题可以放在一条线上看：一个 path 最终怎样变成磁盘 block，metadata 放在哪里，文件增长时又如何同时照顾小文件、大文件和 locality。

FAT 代表简单的 linked-list allocation；inode/FFS 代表 indexed allocation 加 locality；NTFS 则把 MFT、attribute 和 extent 组织到同一套框架里。读这讲时，不妨一直盯住文件系统的基本任务：把原始 blocks 包装成 files/directories，同时在随机访问、顺序访问、空间利用和恢复成本之间取舍。

## 本讲地图

| 主题 | 要解决的问题 | 关键机制 |
| --- | --- | --- |
| FAT | 怎样用简单表结构把文件串成 blocks | File Allocation Table、linked list、directory entry |
| Unix inode | 怎样让随机访问和大文件扩展更好 | inumber、inode、direct / indirect pointers |
| Berkeley FFS | 怎样减少 seek 和 fragmentation | block groups、bitmap、locality、free-space reserve |
| Links and directories | 名字如何绑定到文件，路径如何解析 | hard link、symbolic link、directory traversal |
| NTFS | 怎样把 metadata、small data 和 extents 统一组织 | MFT、attributes、resident data、B-tree directories |

## 正文

文件系统布局本质上是在回答“给定一个 path，怎么找到数据块”。FAT、inode、FFS 和 NTFS 只是给出了不同的索引和局部性方案。
