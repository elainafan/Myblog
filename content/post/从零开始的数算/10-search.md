...
概率分布必须与应用相符。缺少其他信息时可以暂按等概率计算；热点键集中时，把高频记录放在更早位置会改变顺序检索的平均代价。
...
若要寻找第一个等于 `key` 的位置，遇到相等也不能立即返回，而要继续收缩右边界。寻找最后一个等值位置则对称地收缩左边界。闭区间与半开区间应分别保持各自的不变量。
...
索引表使用二分检索时，代价变为 $O(\log b+s)$ 。增大块数会缩短块内扫描，却扩大索引并增加分裂、合并次数。
...
集合的交、并、差可以逐机器字执行按位与、或和与非。位向量空间由全集大小决定，与集合实际元素数无关，因此不适合范围巨大且十分稀疏的键。
...
散列函数要让关键码的各部分都能影响最终地址，使常见输入在表中近似均匀分布。具体写法取决于关键码类型与实际分布，同一个公式换一批数据后未必仍然合适。
...
数字分析法先统计一批已知关键码各位数字的分布，再选取变化较均匀、相关性较弱的数位组成地址。它依赖样本分布，关键码来源改变后必须重新检查。

![数字分析法比较关键码各数位的取值分布](assets/10-digit-analysis.png)

平方取中法先计算关键码平方，再截取中间若干位。平方中的交叉项会把原关键码不同位置的信息带入中部，但截取宽度仍应由表长决定，也不能保证任意输入都均匀。

基数转换法把关键码按某个进制解释，再改用另一个进制取出地址。例如把十进制数字串看作十三进制数，可以打破原数位与十进制周期的直接关系。

![基数转换法把关键码改按另一进制解释后取得散列地址](assets/10-radix-conversion.png)

折叠法把较长关键码切成等长小段，再将各段相加。移位叠加保持各段方向相同，分界叠加则把相邻段交替翻转后求和，避免同一位置总由原关键码的固定数位贡献。

![折叠法通过移位叠加或分界叠加合并关键码分段](assets/10-folding.png)
...
均匀散列假设下，桶的平均长度为 $\alpha$ ，检索的期望时间为 $O(1+\alpha)$ 。链地址法可以直接删除结点，负载因子也允许超过一；代价是额外指针、动态分配和较差的缓存局部性。
...
![双重散列使用第二个散列函数产生与表长互素的探查步长](assets/10-double-hashing.png)
...
设表长为七，散列函数为 $h(k)=k\bmod 7$ 。依次插入 `10, 17, 24` 时，三者初始地址都为三。线性探查会把它们放到位置 `3, 4, 5`。删除 `17` 后若把位置四标成空，检索 `24` 在位置四便会错误停止。墓碑说明当前位置没有记录，但探查链仍未结束。

插入时要记住遇到的第一个墓碑，却不能立刻停下，因为后面可能已有相同键。直到找到空槽或原键后，才决定复用墓碑还是更新旧记录。
...
探查至多访问表长个槽位。若循环没有这个上限，装满或没有空槽的表会陷入死循环。
...
![Rabin-Karp 滚动散列删除窗口首字符并加入新字符](assets/10-rabin-karp.png)
